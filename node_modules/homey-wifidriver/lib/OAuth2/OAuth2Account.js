...
class OAuth2Account extends EventEmitter {
	...
	constructor(options) {
		...
		this.oauth2Url = options.url || options.oauth2Url;
		...
		if (this.refreshingEnabled && options.refreshToken) Homey.ManagerSettings.set(`oauth2_${this.id}_refreshToken`, options.refreshToken);
		...
		if (this.refreshingEnabled) this._refreshToken = Homey.ManagerSettings.get(`oauth2_${this.id}_refreshToken`) || options.refreshToken;
	...
	_processNewAccessTokens(body, resolve, reject) {
		...
		if (!body || !body.hasOwnProperty('access_token') || (this.refreshingEnabled && !body.hasOwnProperty('refresh_token'))) {
