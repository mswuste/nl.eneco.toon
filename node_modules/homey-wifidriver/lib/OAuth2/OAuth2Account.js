...
class OAuth2Account extends EventEmitter {
	...
	constructor(options) {
		...
		this._driverLogMethod = options.log;
		...
		this.log = function () {
			...
			this._driverLogMethod.apply(null, args);
		};

		this.log('new OAuth2Account constructed');
	}
	...
	destroy() {
		...
		this.log(`OAuth2Account ${this.id} is destroyed`);
	}
	...
	getAccessTokens(code) {
		this.log(`get access tokens (code: ${code})`);
		return new Promise((resolve, reject) => {
			request({
				...
				.catch(err => {
					this.log('failed to fetch access tokens', err);
					return reject(err);
				});
		});
	}
	...
	setRefreshTokenTimeout(timeout) {
		this.log(`set refresh token timeout for ${timeout / 1000} seconds`);
	 ...
	 */
	refreshAccessTokens() {
		this.log('refresh access tokens');
		return new Promise((resolve, reject) => {
			...
			request({
				...
				.then(body => this._processNewAccessTokens(body, resolve, reject))
				.catch(err => {
					this.log('failed to refresh access tokens', err);
					return reject(err);
				});
		});
	}
	...
	_processNewAccessTokens(body, resolve, reject) {
		...
		this.log('fetched new access tokens');
		...
		this.emit('tokens', { accessToken: this.accessToken, refreshToken: this.refreshToken });
