...
class OAuth2Client extends EventEmitter {
	...
	constructor(options) {
		...
		this.log('new OAuth2Client constructed');
	}
	...
	createAccount(options) {
		...
		options.log = this._driverLogMethod;
	 ...
	 * accounts list.
	 * @param {OAuth2Account} account_ - OAuth2Account to be deleted
	 */
	deleteAccount(account_) {
		account_.destroy();
		this._accounts = this._accounts.filter(account => account.id !== account_.id);
		this.log(`OAuth2Account (id: ${account_.id}) was destroyed (number of accounts: ${this._accounts.length + 1}`);
	}
}
