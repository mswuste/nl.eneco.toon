...
const WebAPIDriver = require('./../WebAPI/WebAPIDriver');
 ...
 *	}
 */
class OAuth2Driver extends WebAPIDriver {
	...
	customOAuth2OnPair(socket) {
		...
		// Lister for changes of tokens
		oauth2Account.on('tokens', () => {
			this.setDefaultHeaders({
				Authorization: `Bearer ${oauth2Account.accessToken}`,
			});
		});
				...
				.then((devices = []) => {
					...
					devices.forEach(device => {
						...
						if (!device.store.hasOwnProperty('tempOAuth2Account')) {
							...
							// Store only important properties of account
							device.store.tempOAuth2Account = oauth2Account.getStoreValues();
