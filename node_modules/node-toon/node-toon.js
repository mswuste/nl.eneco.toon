...
class Toon extends EventEmitter {
	...
	_parseBody(body) {
		...
		try {
			...
			Log.captureException(new Error('Toon API responded with a body that could not be parsed as JSON'));
			return false;
		}
		...
		return new Promise((resolve, reject) => {
			...
			request(options, (error, response, body) => {
				...
				if (!error && response.statusCode === 200) {
					...
					if (options.url === 'https://api.toonapi.com/toon/api/v1/temperature') {
						Log.captureException(new Error('Toon API responded with Internal Server Error after setting temperature'));
						return reject((error || response.statusCode));
					}

					Log.captureException(new Error('Toon API responded with Internal Server Error'));

					// TODO test set agreement
					this.setAgreement(this.agreementId).then(() => {
						...
						console.log('node-toon: after retry set agreement fail');
					});

					// return reject((error || response.statusCode));

				} else if (error || (response.statusCode !== 200 && response.statusCode !== 401 && response.statusCode !== 500)) {
					if (options.url === 'https://api.toonapi.com/toon/api/v1/temperature') {
						Log.captureException(new Error('Toon API responded with an unknown error after setting temperature'));
						return reject((error || response.statusCode));
					}
					Log.captureException(new Error('Toon API responded with an unknown error'));
					return reject((error || response.statusCode));
				}
			});
		});
