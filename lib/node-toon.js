...
class Toon extends EventEmitter {
	...
	constructor(key, secret) {
		...
		if (!key || !secret) Log.captureException(new Error('No client key or secret found in environment variables'));
	...
	updateState(state) {
		...
		console.log(`node-toon: set state to ${state}`);
	...
	/**
	 * Resume the temperature program.
	 * @returns {*}
	 */
	resumeProgram() {

		console.log('node-toon: resume program');
	...
	_parseBody(body) {
		...
		try {
			...
			console.error('node-toon: error parsing body', body);
			return false;
		}
